...
public class QueryBuilderTest {
    ...
    /**
    * Sets up the needed variables for the QueryBuilderTest
    * Throws an exception upon error in doing so
    */
    @Before
    public void setUp() throws Exception {
        user = "evansalz";
        pass = "830366797";
        queryBuild = new QueryBuilder(user, pass);
        testNames.add("29130,Papa Westray Airport");
        testNames.add("29134,Westray Airport");
        testNames.add("11195,Strayhorn Ranch Airport");
        
        queryTest += "SELECT airports.name, airports.code, airports.id, airports.municipality, "
            + "regions.name, countries.name, "
            + "continents.name FROM continents INNER JOIN countries "
            + "ON continents.code = countries.continent"
    ...
    public void queryTerm() throws Exception {
        ...
        names = queryBuild.queryTerm("stray");
