...
* *349 Remove Destinations:* A user can remove destinations when they come up in "selected" box, after selecting destinations, and have them removed from the itinerary they're setting up to build.  However, this is the only place that the destinations can be removed from and the only time in the process that it can be done.
...
* *493 Map with zoom and pan:* A user was able to zoom and pan on a map provided by Google Docs during the last Sprint Increment, but a user can now save a .KML file of the map, with a button provided on the webpage.
